package com.food.ordering.system.service.domain.outbox.scheduler;
...
public class OrderOutboxScheduler implements OutboxScheduler {
    ...
    public void processOutboxMessage() {
        ...
        if (outboxMessagesResponse.isPresent() && outboxMessagesResponse.get().size()> 0){
            List<OrderOutboxMessage> orderMessages = outboxMessagesResponse.get();
            log.info("Receved {} OrderOutboxMessage with ids: {}, sending to kafka!",orderMessages.size(),
                    orderMessages.stream().map(outboxMessage ->
                            outboxMessage.getId().toString()).collect(Collectors.joining(",")));

            orderMessages.forEach(orderOutboxMessage ->
                    paymentResponseMessagePublisher.publish(orderOutboxMessage,
                            orderOutboxHelper::updateOutboxMessage));
            log.info("{} OrderOutboxMessage sent to message bus! ", orderMessages.size());
        }
